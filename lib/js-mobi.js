...
var jsMobi = (function (W) { // IIFE
    ...
    function force() {
        if (self.any() && !W.location.href.match(mini)) {
            if (W.confirm('You seem to be using a handheld device. Do you want the mobile site?')) {
                W.location = mini;
            }
        } else if (!self.any() && W.location.href.match(mini)) {
            W.location = home;
        }
    }
